...
int transport_open(const char* host, int port)
{
	...
	// Get APN
	char APN[SIZE_APN];
	sprintf(APN, "\"%s\",\"%s\",\"%s\"\r", GetVariable("ANAME"),
			GetVariable("AUSER"), GetVariable("APSWD"));

#ifdef HYBRID_M95_WLAN_CODE
	wlanRecvStopMQTT();
	HAL_UART_Receive_DMA(&HUART_GPRS, DataBuffer->buffer, DataBuffer->size);
	Reset(DataBuffer);
#endif
...
int transport_get_time(void)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		for (j = 0; j < 5 && Stock(DataBuffer) < 4; j++)
			HAL_Delay(1000);

		if (Stock(DataBuffer) < 4) continue;

		// Parse time
		uint64_t time_bin = Read(DataBuffer);
		time_bin = (time_bin << 8) | Read(DataBuffer);
		time_bin = (time_bin << 8) | Read(DataBuffer);
		time_bin = (time_bin << 8) | Read(DataBuffer);

		if (time_bin < 2208988800ul) continue;

		time_t t = time_bin - 2208988800ul;
		struct tm *lt = gmtime(&t);
		size_t len = strftime(buf, 256, "%Y/%m/%d,%H:%M:%S", lt);
...
int transport_update_tz(void)
{
	...
	for (j = 0; j < 3; j++)
	{
		...
		while (Stock(DataBuffer) && rbufleft)
		{
			*rbufptr++ = Read(DataBuffer);
			rbufleft--;
		}
...
#ifdef COMMUNICATION_M95
	if (sock == 1){
		if (HAL_UART_Transmit(&HUART_GPRS,buffer,lengthBuffer,3000)==HAL_OK) return 1;
		else return -1;
	}
	else {
...
M95Status Modem_Init() {
	...
	M95Status rc = M95_Initialize( &HUART_GPRS,WDT_ENABLED, &hiwdg,
			M95_CTRL_EMERG_GPIO_Port, M95_CTRL_EMERG_Pin,
			M95_CTRL_PWRKEY_GPIO_Port, M95_CTRL_PWRKEY_Pin,
			M95_STATUS_GPIO_Port, M95_STATUS_Pin,
...
M95Status ConnectPlus(
	...
	if (!strcmp(CommandList[i].id,"END")){
		...
		if (0) {
			...
			/*int num =*/ executeCommand (&DeactCmd, &HUART_GPRS);

		}
		if (1) {
			/*int j =*/ DisconnectPlus (WDT_ENABLED,hiwdg, &HUART_GPRS, timeoutGPRS, 1000, rebootSystem);
		}
		if (0) {
