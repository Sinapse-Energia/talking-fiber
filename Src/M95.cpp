...
int	QuectelM95::ConnectTCP(const char *apn, const char *host, int port){
	...
	char 	CTX_TX[100]; 	// to build QIFGCNT (CTX)
	...
	sprintf(CTX_TX, "AT+QIFGCNT=%d\r", context);
...
//			{	ATMATCH,	"",		{"AT+QIFGCNT=0\r"}, {"\r\nOK\r\n"},				{ 300, 0},  1} ,
			...
			{	ATMATCH,	"",		{mode?"AT+QIMODE=1\r":"AT+QIMODE=0\r"},{"\r\nOK\r\n"}, {1000, 0}, 	1} ,
...
int	QuectelM95::ConnectTLS(const char *apn, const char *host, int port){
	...
	sprintf(GPRS_TX,	"AT+QICSGP=1,%s\r", apn);				// the 1 means GPRS Bearer
	sprintf(NTP_TX, "AT+QNTP=%s\r", const_SERVER_NTP);

								//	sid, context
	sprintf(OPEN_TX,	"AT+QSSLOPEN=1,0,\"%s\",%d,%d\r", host, port, mode);
...
#endif
#endif
// 4-B) Device RTC read and process
	{	ATGET,		"",		{"AT+CCLK?\r"}, 					{NULL,	NULL, SetDateTime},	{1000, 0}, 	1,  },
// 5) CFG
	// next 3, the first parameter is context
...
int	QuectelM95::SynchronizeTime(){
	char 	NTP_TX[100]; 	// to build QINTP AT command
	sprintf(NTP_TX, "AT+QNTP=%s\r", const_SERVER_NTP);
	CmdProps CmdSynch[] = {
		{ATMATCH,"",	{NTP_TX,}, 			{"\r\nOK\r\n\r\n+QNTP: 0\r\n"},	{1000, 0, 10000}, 1, },
