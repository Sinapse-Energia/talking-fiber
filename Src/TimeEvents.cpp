...
//	long timestamp = ((h * 60) + m )* 60 + s;   // timestamp to be executed
	long timestamp = GetTimeStampAt(h, m, s);
...
#ifdef DEBUG
char  *CTimeEvent::tostring	(){
	sprintf (work, "uno de %s para %ld %d veces\n", id, value, times);
	return work;
}

char	*CTimeEvent::Show (const char *x) {
...
char	*ShowTimers(const char *x){
	return CTimeEvent::Show(x);
}
#endif
